...
import { fetchCopyDetails } from '../../utils/fetchCopyDetails';

const LrUpdateCopy = ({copyid, companyId}) => {
  ...
  //   try {
  //     const response = await axiosInstance.get(`/api/lr/specific/${copyid}`)
  ...
  const updateLr = async () => {
    try {
      setIsLoading(true)
      const response = await axiosInstance.patch(`api/lr/update/${copyid}`, data)
      setIsLoading(false)
  ...
  useEffect(() => {
    fetchCopyDetails('lr',copyid, setData)
  }, [])
            ...
            <div className="p-2  text-xs h-36">
              ...
              <div className="pt-[19px]">
                <div className="border border-black p-1 mt-1 h-32">
                  ...
                  {/* caution.consignment_note.consignment_date */}
                  <p>
                    Date :
                    <input
                      className="border-b-2 outline-none bg-transparent text-center"
                      ...
                      onChange={(e) => handleMoreNestedChange("caution.consignment_note.consignment_date",e.target.value, "date")}
